...
| **Заметки** | Временные, в правом столбце иконок: быстро записать, вернуться, удалить или унести через буфер обмена. Наведение сразу даёт каретку, пустые заметки убирают себя сами. Заметку можно превратить в список дел и обратно |
| **Usage** | Живые лимиты Claude, Codex, Cursor и OpenCode. Ненужные карточки выключаются в настройках — три остаются в один ряд, четыре складываются 2×2 |
| **Помодоро** | Фокус и перерыв из выреза; пока сессия идёт, свёрнутый островок несёт цвет фазы |
| **Память** | Тот же график давления, что в «Мониторинге системы» (зелёный / жёлтый / красный), плюс список кэшей, которые безопасно снести. Вырез светится **только на красном** — жёлтый на графике это обычный рабочий фон, особенно на 16 ГБ. Свечение от процента RAM — опция в настройках, по умолчанию выкл. |
| **Цвета** | Пипетка; значение копируется как HEX |
...
Иконка в меню-баре — переключить панель, открыть настройки, idle-экран,
скрыть содержимое, выйти. Вкладки на рельсах можно прятать из полного окна
настроек (пресеты Work / Call / Minimal).
...
swift run CyclopLogicCheck   # свечение и сетка Usage, без XCTest
...
Доступ к Календарю — единственное разрешение, которое Cyclop просит, чтобы
вкладка заработала. Оно нужно только «Календарю», и системный диалог
показывается не при запуске и не при открытии вкладки, а по явному нажатию
кнопки на объясняющем экране. Не пользуешься календарём — этого запроса не
будет.

Очистка системных кэшей на вкладке «Память» — второе, необязательное:
полный доступ к диску. Без него остальной список всё равно работает; панель
говорит об этом и предлагает кнопку в настройки. Уведомления за десять минут
до созвона и в конце помодоро — третье: macOS спросит, когда придёт первое.
...
```
Sources/CyclopLogic            правило свечения и сетка Usage — с тестами
Sources/Cyclop
